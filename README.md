...
- 공개 상태: 실제 도메인 미정, `.invalid` canonical + `noindex,nofollow`
...
실제 도메인이 확정되기 전에는 검색엔진에 제출하지 않습니다. 공개할 때는 canonical·OG·sitemap·robots를 실도메인으로 한 번에 전환하고 `index,follow`를 적용한 뒤 `pnpm verify`를 다시 실행해야 합니다.
