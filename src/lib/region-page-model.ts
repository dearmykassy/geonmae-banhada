...
export function createRegionPageModel(node: RegionNode): RegionPageModel {
  ...
  if (!directory || directory.id !== "local-boundary") {
    throw new Error(`GEONMAE_BANHADA_REGION_DIRECTORY_SECTION_MISSING:${node.path}`);
  }
  if (content.hooks.length !== 2 || content.ctaLabels.length !== 3) {
    throw new Error(`GEONMAE_BANHADA_REGION_COPY_SHAPE_INVALID:${node.path}`);
  }
  ...
  const galleryHeading =
    children.length > 0
      ...
      : `${node.displayName} 주소 세부 확인`;
  const gallerySummary =
    children.length > 0
      ? `${children.length}개 하위 지역`
      : "상세 주소를 통화에서 확인하는 지역";
  const galleryTerminal = "받을 곳의 도로명과 건물명, 가능한 시간은 통화로 전달합니다.";
  const sceneIndex = `REGION INFO · ${node.segments.length
    .toString()
    .padStart(2, "0")}`;
  const sceneCaption = "받을 곳의 주소와 가능한 시간을 통화에서 맞춥니다.";
  ...
  if (headingViolations.length > 0) {
    throw new Error(
      `GEONMAE_BANHADA_REGION_ADJACENT_HEADING_SEMANTIC_DUPLICATE:${node.path}:${headingViolations[0].actionId}`,
    );
  }
  ...
  const renderedSurface: RegionRenderedCopyEntry[] = [
    ...
    entry("gallery:index", "AREA DIRECTORY", "decorative"),
    ...
    entry("final:heading", "통화 전에 받을 주소와 시간을 적어 주세요", "owned-copy"),
  ...
  if (new Set(ids).size !== ids.length) {
    throw new Error(`GEONMAE_BANHADA_REGION_RENDER_COPY_ID_DUPLICATE:${node.path}`);
  }
  ...
  return {
    ...
    finalBeat: {
      label: "24시간 전화상담",
      heading: "통화 전에 받을 주소와 시간을 적어 주세요",
