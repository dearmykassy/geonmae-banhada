...
if (
  ...
  throw new Error("GEONMAE_BANHADA_CAPITAL_REGION_SOURCE_INVALID");
}
...
if (
  ...
  throw new Error("GEONMAE_BANHADA_SERVICE_REGION_SOURCE_INVALID");
}
...
function nodeForSegments(segmentsInput: readonly string[]): RegionNode | null {
  ...
  return {
    id: `geonmae-banhada-region-${segments.map(encodeURIComponent).join("-")}`,
    kind,
    rootKey,
    segments,
...
if (ACTIVE_REGION_NODES.length !== 1291) {
  throw new Error(
    `GEONMAE_BANHADA_ACTIVE_ROUTE_COUNT_INVALID:${ACTIVE_REGION_NODES.length}`,
  );
}
...
export function getRegionOrdinal(node: RegionNode): number {
  ...
  if (ordinal === undefined) {
    throw new Error(`GEONMAE_BANHADA_REGION_ORDINAL_MISSING:${node.path}`);
  }
  return ordinal;
}
...
export function getRootNode(rootKey: ActiveRootKey): RegionNode {
  ...
  if (!node) throw new Error(`GEONMAE_BANHADA_ROOT_NOT_FOUND:${rootKey}`);
  return node;
}
