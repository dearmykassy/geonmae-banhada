...
function createAssetIndexes(): number[] {
  ...
  for (let nodeIndex = 0; nodeIndex < assignments.length; nodeIndex += 1) {
    ...
    if (swapIndex < 0) {
      throw new Error(
        `GEONMAE_BANHADA_REGION_IMAGE_ASSIGNMENT_UNRESOLVED:${ACTIVE_REGION_NODES[nodeIndex].path}`,
      );
    }
...
export function getRegionalImageAssetNumber(node: RegionNode): number {
  ...
  if (nodeIndex === undefined) {
    throw new Error(`GEONMAE_BANHADA_REGION_IMAGE_NODE_MISSING:${node.path}`);
  }
  return ASSET_INDEXES[nodeIndex] + 1;
}

export function getRegionalImageAssetId(node: RegionNode): string {
  return `gmb-t4-rgn-${String(getRegionalImageAssetNumber(node)).padStart(3, "0")}-v1`;
}
...
export function getRegionalImageAssetPath(
  ...
  return `/assets/geonmae-banhada/template4-regional/${getRegionalImageAssetId(node)}/${variant}.webp`;
}
